...
let legacyDailyLogIndexChecked = false;

async function dropLegacyDailyLogIndex() {
  if (legacyDailyLogIndexChecked) return;
  legacyDailyLogIndexChecked = true;

  try {
    await DailyLog.collection.dropIndex('userId_1_date_1');
  } catch (error) {
    if (error?.codeName !== 'IndexNotFound' && error?.code !== 27) {
      console.warn('Could not drop legacy DailyLog user/date index', error);
    }
  }
}
